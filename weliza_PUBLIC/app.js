...
function setupContactForm() {
  ...
  if (form) {
    form.addEventListener('submit', (e) => {
      ...
      const subject = encodeURIComponent(`B2B Stitching Inquiry from ${name}`);
      const body = encodeURIComponent(
        `Weliza Inquiry Details:\n\n` +
        `Name/Company: ${name}\n` +
        `Email: ${email}\n` +
        `Phone: ${phone}\n` +
        `Role: ${interest}\n\n` +
      ...
      window.open(`mailto:info@weliza.com?subject=${subject}&body=${body}`, '_blank');
      
      alert('Thank you! Your email client has been opened with your inquiry prefilled.');
      form.reset();
    });
  }
}
...
function setupModal() {
  ...
  window.openQuickView = function(productId) {
    ...
    if (modal) {
      ...
      modalEmailBtn.href = `mailto:info@weliza.com?subject=${emailSubject}&body=${emailBody}`;
